  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <Card title="The pipeline — who does what" subtitle="Every stage has one owner, one required action, and one completion rule.">
      <div className="space-y-3">
        ...
        <PipelineStep stage="Connected" owner="Agent" action={`Diagnose the visible website problem, its business consequence, authority, timing, and minimum ${usd(WEBSITE_PACKAGES.starter.setupFloor)} setup plus ${usd(WEBSITE_PACKAGES.starter.monthlyFloor)}/month willingness.`} done="All four qualification gates are confirmed or the lead is marked Lost." />
        ...
        <PipelineStep stage="Paid → Won" owner="Founder or closer" action="Verify the exact live Stripe Checkout collection, or have a founder attest to cleared manual funds, and select the assigned builder." done="The full setup amount is verified, the lead enters Won, and one accrued commission row is created for each credited person." />
        <PipelineStep stage="Won → Onboarding" owner="Admin or assigned builder" action="Confirm the paid deal is ready to hand to delivery, then advance it once." done="The prepared brief and assigned builder move into the active Onboarding lane." />
    ...
    <Card title="Role growth path" subtitle="Launch V1 stays simple without boxing the company in.">
      <p className="text-sm leading-relaxed text-fg-muted">Every new sales hire starts as an <b className="text-fg">opener</b>, earning {pct(COMPANY_TRACK_BPS.opener / 10000)} on collected setup revenue. Their scorecard is conversations, qualified meetings booked, show rate, and handoff quality. A proven Agent is then granted the <b className="text-fg">closer track</b>: closing a company-provided lead earns {pct(COMPANY_TRACK_BPS.closer / 10000)}, whether or not a separate opener is credited, while finding and closing the lead earns {pct(SELF_TRACK_BPS.open_close / 10000)}. The track is granted per rep by CC or Adon and is never implied by tenure.</p>
    </Card>
    ...
    <Card title="Rep compensation" subtitle="Percent of collected setup revenue · paid on cash collected · no recurring share in V1.">
      <p className="text-sm leading-relaxed text-fg-muted mb-4">The standard ladder is simple: <b className="text-fg">open {pct(COMPANY_TRACK_BPS.opener / 10000)}</b>, <b className="text-fg">close {pct(COMPANY_TRACK_BPS.closer / 10000)}</b>, or <b className="text-fg">find and close {pct(SELF_TRACK_BPS.open_close / 10000)}</b>. These are the same numbers as in your agreement and the same numbers the payout runs on — every surface reads from one engine.</p>
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <RateCard role="You close it" rate={pct(COMPANY_TRACK_BPS.closer / 10000)} detail={`Close a company-sourced lead. If a separate opener is credited, their ${pct(COMPANY_TRACK_BPS.opener / 10000)} is recorded independently; your closer rate stays ${pct(COMPANY_TRACK_BPS.closer / 10000)}.`}/>
      ...
      <p className="text-sm leading-relaxed text-fg-muted mt-3">Every standard offer from {usd(SPECIALIST_SPLIT_FLOOR_CENTS / 100)} upward can credit a separate opener and closer. A founder-approved exception below the Starter price is full-stack only.</p>
