...
/**
 * marketing_review.decision has its own CHECK constraint — approve / approve_with_changes
 * / request_changes / reject / comment — and it is NOT the asset status. Inserting
 * "approved" or "archived" violates it, and because this insert is best-effort by design
 * the whole audit trail then fails silently.
 *
 * DEDUPE ON MERGE: Bravo has `reviewDecisionFor` in lib/founders-marketing-core.ts on his
 * branch; it is not on main yet, so this branch carries its own copy to stay mergeable.
 * When his lands, delete this and import his — two mappings of one CHECK constraint is
 * exactly how they drift apart.
 */
const decisionFor = (status: AssetStatus): string | null => {
