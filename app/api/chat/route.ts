...
export async function POST(req: NextRequest) {
  ...
  const searchFirstBridge = isOperator
    ? `\n\nSEARCH BEFORE DECLINING — NON-NEGOTIABLE:\nBefore you EVER say "I don't have X" or "I don't see X anywhere" or ask the operator for something they might have already saved, you MUST:\n  1. Check the OPERATOR KNOWN FACTS block (if present below) — calendar links, signatures, business names, common assets live there.\n  2. If still not found, call read_brain_doc on a plausibly-named doc (e.g., USER.md, STATE.md, PROFILE.md) or search_memory with relevant keywords.\n  3. Only after a real search returns nothing should you ask the operator. When they give you the answer, IMMEDIATELY call save_known_fact to persist it so the next chat (and every chat after that) already knows.\nThe operator gets visibly frustrated when you decline without searching — they've told previous instances of you the same thing 20 times. Search first, ask last, save always.`
    : `\n\nSEARCH BEFORE DECLINING:\nBefore saying "I don't have X" or asking the operator for something basic about them, ALWAYS check the OPERATOR KNOWN FACTS block (if present below) — that's where they save evergreen facts. If a fact isn't there, ask AND call save_known_fact with their answer so the next chat doesn't have to ask again.`;
  ...
  const searchFirstNoBridge = isOperator
    ? `\n\nBEFORE DECLINING — CHECK KNOWN FACTS:\nEven without the bridge, the OPERATOR KNOWN FACTS block (if present below) and search_memory can answer most "what's my X?" questions. ALWAYS check those first before saying "I don't have X." If a fact you need is missing, ask the operator AND call save_known_fact with their answer so the next chat doesn't have to ask again.`
    : `\n\nBEFORE DECLINING — CHECK KNOWN FACTS:\nThe OPERATOR KNOWN FACTS block (if present below) holds evergreen facts the operator has saved. ALWAYS check it before saying "I don't have X." If a fact isn't there, ask AND call save_known_fact with their answer so the next chat knows it.`;
