...
/**
 * LeadFileBody — presentational body of the lead/application file.
 *
 * Extracted from LeadDetailDrawer.tsx (2026-07 conversations-inbox-v2 work)
 * so the same UI can render inline in the Conversations context panel AND
 * inside the slide-in drawer. This component does NOT fetch data and does
 * NOT own open/close state — the caller (LeadDetailDrawer, or the inbox
 * context panel) owns the fetch + reload + close affordance and passes the
 * loaded record/documents/application down as props.
 */
...
export function LeadFileBody({
  ...
  return (
    // `relative` is required here (not just on the historical drawer <aside>
    // that used to wrap this JSX): DrawerFooter's composer overlays
    // (ComposerShell) are `absolute inset-0`, and this component now also
    // renders inline inside ContextPanel's scrolling stack, which has no
    // positioned ancestor of its own. Anchoring the overlay to THIS root
    // keeps it correct in both hosts.
    <div className="relative flex flex-col h-full">
      ...
      <div className="shrink-0 px-5 py-4 border-b border-bg-border/60 space-y-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <StagePicker
              ...
              entity={entity}
              currentStage={
                String(record[entity === "application" ? "status" : "stage"] || "") || null
              }
              onChanged={onReload}
            />
            {onClose && (
        ...
        {/* Row 2: Stat tiles */}
        <StatTiles record={record} application={application} />

        {/* Row 3: Owner/Signer + Assigned to */}
        <OwnerAssignedRow record={record} />
        ...
        <CollaboratorsControl
          ...
          currentCollaborators={
            Array.isArray(record.collaborators)
              ? (record.collaborators as unknown[]).filter(
        ...
        {entity === "lead" && (
          <TransferControl
            ...
            leadId={recordId}
            transferred={Boolean(record.transferred_at)}
            applicationId={
              application?.id ||
              (typeof record.application_id === "string" ? (record.application_id as string) : null)
            }
            onTransferred={onReload}
          />
        )}
        ...
        {entity === "application" && (
          <ReturnControl
            ...
            hasLead={typeof record.lead_id === "string"}
            onReturned={onClose ?? onReload}
          />
        )}
