    ...
    if (error.code === "workspace_calendar_token_invalid") {
      console.error(
        "      The shared workspace credential was rejected. A host reconnecting will NOT help — " +
          "securely sync the existing verified workspace bundle from the canonical secret store. " +
          "Rotate only if that canonical bundle also fails direct provider verification.",
      );
    }
    process.exit(1);
  }
