 ...
 *      rules (lib/delivery/rules.ts parseSupportSubmission).
 *   3. Stores the optional attachment in private object storage under the
 *      ticket id (never the lead-documents path: a ticket is not a lead).
 ...
 *   5. Matches the client by email (server-side only, never echoed back).
 *   6. Creates the ticket, keyed on the submission id (idempotent).
 *   7. After the response: founders' Telegram + email, and the client's
...
export async function handleSupportFormSubmission(
  ...
  const ipLimit = rateLimit({ key: `support-form:${ip}`, capacity: 5, refillPerSec: 5 / 60 });
  if (!ipLimit.allowed) return fail(429, "rate_limited", { retry_in_sec: ipLimit.resetIn });
  ...
  const emailLimit = rateLimit({ key: `support-form-email:${sub.email}`, capacity: 3, refillPerSec: 3 / 600 });
  if (!emailLimit.allowed) return fail(429, "rate_limited", { retry_in_sec: emailLimit.resetIn });

  const ticketId = randomUUID();
  const submissionId = randomUUID();

  // 3. Attachment. Only the declared file_upload field, only allowlisted types,
  //    only up to the cap. A failure keeps the ticket and says so on it.
  ...
  for (const [key, value] of Object.entries(payload)) {
    ...
    const up = await (deps.upload ?? defaultUploader)(path, bytes, value.mime_type);
    if (up.ok) {
      attachments.push({ ...meta, size_bytes: bytes.length, storage_path: path });
    } else {
      console.error("[support-intake] attachment upload failed", { ticket: ticketId, error: up.error });
      attachments.push({ ...meta, storage_path: null, error: "upload failed" });
      warnings.push({ field_name: key, reason: up.error });
    }
  }
  if (attachments.length) payload.attachment = attachments;

  // 4. The submission record — the idempotency key for the ticket.
  ...
  await db.execute({
    ...
    args: [
      ...
      resolvedIp === "unknown" ? null : resolvedIp,
      userAgent,
      now.toISOString(),
    ],
  });

  // 5 + 6. Match and create.
  const { ticket } = await createTicketFromSubmission(db, { ticketId, submissionId, sub, attachments }, now);

  // 7. After the response, never before it.
