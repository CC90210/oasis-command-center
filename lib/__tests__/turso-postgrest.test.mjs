...
await client.executeMultiple(`
  ...
  INSERT INTO tenant_records VALUES
    ('p1','t1','lead','{"name":"Acme, Inc.","business_city":"Toronto","stage":"assigned","assigned_to":"rep-1","sales_program":"website_sales_v1"}','2026-01-01T00:00:00Z','2026-08-03T00:00:00Z'),
    ('p2','t1','lead','{"name":"Older shop","business_city":"Acme Industrial Park","stage":"assigned","assigned_to":"rep-1","sales_program":"website_sales_v1"}','2025-01-01T00:00:00Z','2026-01-03T00:00:00Z'),
    ('p3','t1','lead','{"name":"Research only","business_city":"Toronto","stage":"researched","assigned_to":"rep-1","sales_program":"website_sales_v1"}','2026-01-01T00:00:00Z','2026-08-04T00:00:00Z'),
    ('p4','t1','lead','{"name":"Legacy unassigned","stage":"assigned","assigned_to":"","sales_program":"website_sales_v1"}','2026-01-01T00:00:00Z','2026-08-02T00:00:00Z'),
    ('p5','t1','lead','{"name":"Null unassigned","stage":"assigned","sales_program":"website_sales_v1"}','2026-01-01T00:00:00Z','2026-08-01T00:00:00Z');
`);
...
await test("pipeline JSON search filters before exact count + range", async () => {
  const result = await db.from("tenant_records")
    .select("id, data, updated_at", { count: "exact" })
    .eq("tenant_id", "t1")
    .eq("entity_type", "lead")
    .eq("data->>sales_program", "website_sales_v1")
    .eq("data->>stage", "assigned")
    .eq("data->>assigned_to", "rep-1")
    .or("data->>name.ilike.*Acme*,data->>business_city.ilike.*Acme*")
    .order("updated_at", { ascending: false })
    .range(1, 1);
  assert.equal(result.error, null);
  assert.equal(result.count, 2, "count covers every DB match, not only the requested range");
  assert.deepEqual(result.data.map((row) => row.id), ["p2"], "the older match remains reachable on page two");
});

await test("pipeline unassigned scope includes null and legacy empty string", async () => {
  const result = await db.from("tenant_records")
  ...
  assert.deepEqual(result.data.map((row) => row.id), ["p4", "p5"]);
});
...
await test("JSON-path ->> filter compiles to json_extract", async () => {
  const { error } = await db.from("leads").select("id")
    .is("meta->>src", null);
  assert.equal(error, null);
...
// --- JSON-path SELECT projection (lib/web-leads/data.ts's two-phase lead read)
//
// The whole point is the OUTPUT KEY. Compiling to json_extract was never the
// hard part; naming the resulting column the way PostgREST names it is, and
// before selectCol() these rows came back keyed by the raw SQL expression
// (`json_extract("data", '$.business_name')`).
//
// The first THREE tests below were run against the old `.map(q)` compiler and
// watched to fail (2026-08-25). The last two are regression guards and pass in
// both directions BY DESIGN -- they exist to prove selectCol did not change
// plain-column naming or weaken the identifier guard, so a green result from
// them is the point, not an unproven assertion.
await test("select of a JSON path is keyed by the LAST PATH SEGMENT, as PostgREST names it", async () => {
  const { data, error } = await db.from("leads").select("id,meta->>src").eq("id", "l1");
  assert.equal(error, null);
  assert.deepEqual(Object.keys(data[0]).sort(), ["id", "src"],
    "must be {id, src} — not a key containing json_extract");
  assert.equal(data[0].src, "form");
});

await test("select of a JSON path accepts an explicit alias, which wins", async () => {
  const { data, error } = await db.from("leads").select("id,source:meta->>src").eq("id", "l1");
  assert.equal(error, null);
  assert.deepEqual(Object.keys(data[0]).sort(), ["id", "source"]);
  assert.equal(data[0].source, "form");
});

await test("projecting several JSON paths keeps every key distinct", async () => {
  const { data, error } = await db.from("leads")
    .select("id,meta->>src,meta->>campaign", { count: "exact" })
    .eq("id", "l1");
  assert.equal(error, null);
  assert.ok("src" in data[0] && "campaign" in data[0],
    `both paths must be addressable by name, got ${JSON.stringify(Object.keys(data[0]))}`);
});

await test("plain columns are NOT aliased, so existing call sites are untouched", async () => {
  const { data, error } = await db.from("leads").select("id,email,status").eq("id", "l1");
  assert.equal(error, null);
  assert.deepEqual(Object.keys(data[0]).sort(), ["email", "id", "status"]);
});

await test("a hostile alias is rejected rather than interpolated", async () => {
  const res = await db.from("leads").select('x":: ,(SELECT 1) AS "y:meta->>src');
  assert.ok(res.error, "must not compile an alias containing quotes");
  const still = await db.from("leads").select("id").limit(1);
  assert.equal(still.error, null, "table must still be readable");
});
