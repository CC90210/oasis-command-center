    ...
    describe: (r) => {
      ...
      if (r.observed === INVALID_GRANT) {
        return (
          ...
          "First securely sync the already-authorized workspace credential bundle from the canonical " +
          "secure store into the GOOGLE_SYSTEM_CALENDAR_* deployment slots, then verify it with " +
          "scripts/verify-workspace-calendar-live.ts. Rotate only if that canonical credential also " +
          "fails direct provider verification."
        );
      }
      ...
      return (
        ...
        "a definitive client error that was not safe or useful to persist verbatim. Securely sync " +
        "the existing verified workspace bundle from the canonical secret store and re-run the live " +
        "verifier. Rotate only if the canonical bundle itself fails direct provider verification."
      );
    },
  }];
}
