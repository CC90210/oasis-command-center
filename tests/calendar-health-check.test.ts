...
async function run() {
  try {
    ...
    assert.doesNotMatch(revoked, /mint|fresh credential/i, "invalid_grant alone must not prescribe replacing a credential");
    assert.match(revoked, /existing|secure store|sync/i, "the remedy must start from the existing secure credential store");
 ...
 */
{
  ...
  assert.doesNotMatch(verifier, /must mint|mint a fresh/i, "the verifier must not turn a rejection into unsupported rotation advice");
}
